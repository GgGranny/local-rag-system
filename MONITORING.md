...
RAGAS 0.4.3 has an upstream compatibility defect: it imports an obsolete optional VertexAI module from `langchain-community` before it initializes any evaluator. LangChain 1.x removed that module, so the local service installs a narrowly scoped inert import marker before importing RAGAS. It does not configure, call, or fall back to VertexAI or any cloud provider. Remove this shim after upgrading to a RAGAS release that fixes the optional import.

RAGAS 0.4.3 uses `SingleTurnSample(user_input=<original question>, response=<generated answer>)` and `single_turn_ascore`. The application probes that installed API at evaluation time and records a safe `FAILED` result if it cannot be imported or is incompatible; this never affects chat. The service contains a local compatibility shim for the current RAGAS/LangChain optional-Vertex import defect described above. No cloud model is used.
